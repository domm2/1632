...
public class BeanCounterLogicImpl implements BeanCounterLogic {
	...
	BeanCounterLogicImpl(int slotCount) {
		// impl = new BeanCounterLogicImpl(slotCount);
		this.slotCount = slotCount;
		inFlightBeans = new ArrayList<Bean>();
		remainingBeans = new ArrayList<Bean>();
		inSlotsBeans = new ArrayList<List<Bean>>(this.slotCount);
		for (int i = 0; i < this.slotCount; i++) {
			inSlotsBeans.add(new ArrayList<Bean>());
		}
	}
	...
	public int getInFlightBeanXPos(int yPos) {
		for (int i = 0; i < inFlightBeans.size(); i++) {
			Bean bean = inFlightBeans.get(i);
	...
	public int getSlotBeanCount(int i) {
		return inSlotsBeans.get(i).size();
	}

	public int getInFlightBeanCount() {
		return inFlightBeans.size();
	}
	...
	public double getAverageSlotBeanCount() {
		...
		for (int i = 0; i < slotCount; i++) {
			double beansIn = getSlotBeanCount(i);
			slotxbean += (double) i * beansIn;
		}
		// divide var by 2
		average = (double) slotxbean / (double) beanCount;
	...
	public void upperHalf() {
		...
		// reset beans starting from last slot
		for (int m = 0; m < slotCount; m++) {
			while (del > 0) {
				List<Bean> temp = inSlotsBeans.get(m);
				int tempsize = temp.size();
				if (tempsize > 0) {
					// reset last bean in slot
					temp.get(tempsize - 1).reset();
					// delete last bean from list
	...
	/* returns the amount of beans in all slots */
	private int getInSlotsBeanCount() {
		int inSlots = 0;
		for (int i = 0; i < slotCount; i++) {
			inSlots += getSlotBeanCount(i);
		}
		return inSlots;
	}
	...
	public void repeat() {
		...
		// loop through the slots list. reset each bean then remove it from this list
		// and add to remaining list
		// reset beans starting from last slot
		for (int m = 0; m < slotCount; m++) {
			List<Bean> temp = inSlotsBeans.get(m);
			int tempsize = temp.size();
			for (int i = 0; i < tempsize; i++) {
				// reset every bean in slot
				temp.get(i).reset();
				// add bean to remaining list
				remainingBeans.add(temp.get(i));
			}
			// delete bean from list
			temp.clear();
		}
		// loop through the inflights list. reset each bean then remove it from this
		// list and add to remaining list
		if (inFlightBeans.size() > 0) {
			for (int j = 0; j < inFlightBeans.size(); j++) {
				inFlightBeans.get(j).reset();
				// add bean to remaining list
				remainingBeans.add(inFlightBeans.get(j));
			}
			inFlightBeans.clear();
		}
		if (remainingBeans.size() > 0) {
			inFlightBeans.add(remainingBeans.get(0));
			remainingBeans.remove(0);
		}
		// System.out.println("inflight beans should be one: " + inFlightBeans.size());
		// System.out.println(" ");
	}
	...
	public boolean advanceStep() {
		// call advanceStep() on each of the in-flight Beans
		// to update their X and Y coordinates accordingly.
		...
		for (int i = 0; i < inFlightBeans.size(); i++) {
			inFlightBeans.get(i).advanceStep();
			if (inFlightBeans.get(i).getYPos() >= slotCount) {
				inSlotsBeans.get(inFlightBeans.get(i).getXPos()).add(inFlightBeans.get(i));
				// store i
				tempremove.add(i);
				// inFlightBeans.remove(i);
			}
		}
		// remove those in slots
		for (int c : tempremove) {
			inFlightBeans.remove(c);
		}

		// adds bean to start position
		if (remainingBeans.size() > 0) {
			inFlightBeans.add(remainingBeans.get(0));
			remainingBeans.remove(0);
		}
	 ...
	 */
	@edu.umd.cs.findbugs.annotations.SuppressFBWarnings(value = "VA_FORMAT_STRING_USES_NEWLINE", justification = "I know we should be using %n instead of \n, but JPF for some reason does not like %n")
